//
//  RefreshWalletBalanceService.swift
//  OMP_iOS
//
...
struct RefreshWalletBalanceService: EndPointType {
    
    static let agent = Router<Self>()
    
    var path: String {
        return "/user/wallet/\(requestModel.coin)/deposit/refresh"
    }
...
extension RefreshWalletBalanceService {
    ...
    static func refreshDepositWalletBalance(request: Request, completion: @escaping (Result<NetworkResult<Response, ErrorResponse>, NetworkError>) -> Void) {
